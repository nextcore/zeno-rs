...
pub fn register(engine: &mut Engine) {
    ...
    let for_handler = Arc::new(|engine: &Engine, ctx: &mut Context, node: &Node, scope: &Arc<Scope>| {
        ...
        if let Some(ref val) = node.value {
            if val.contains(';') {
                ...
                // Resolve the list variable directly from scope, bypassing resolve_shorthand_value(node)
                // because node has children (as, do) that would be incorrectly treated as a map.
                let v = val.trim();
                loop_val = if v.starts_with('$') {
                    scope.get(&v[1..]).unwrap_or(Value::Nil)
                } else {
                    engine.resolve_shorthand_value(&Node {
                        name: String::new(),
    ...
    engine.register(
        "forelse",
        Arc::new(|engine, ctx, node, scope| {
            // Resolve the list variable from node.value directly (not via resolve_shorthand_value(node)
            // because node has children which would be treated as a map by the resolver).
            let val = node.value.as_deref().map(|v| {
                let v = v.trim();
                if v.starts_with('$') {
                    scope.get(&v[1..]).unwrap_or(Value::Nil)
                } else {
            ...
            if list.is_empty() {
                ...
                // Synthesize a "for" node, preserving node.value for the list reference
                let for_node = Node {
                    name: "for".to_string(),
